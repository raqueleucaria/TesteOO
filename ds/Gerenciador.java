/**
* Classe responsável pela gerência de temas e palavras. 
*
...
public class Gerenciador {
	...
	public static void gerenciarTemas() {
		...
		do{
			...
			opcao = Utensilios.validarVariavel(1, 4, "Opcao invalida, digite novamente: ");
			System.out.println();
	...
	private static void cadastrarTema() {
		...
		if (Utensilios.arrayContem(temas(), novoTema))
			System.out.println("Tema digitado ja cadastrado!");
		else {
	...
	private static void excluirTema() {
		...
		System.out.printf("%nDigite o numero do tema que deseja excluir: ");
	...
	// Vasculha a array temas, apresentando os temas disponiveis ao usuario
	...
	// Pede ao usuario que escolha o tema e avalia se o valor digitado e valido ou nao.
	public static int escolhaTema() {
		...
		int indexTemaEscolhido = Utensilios.validarVariavel(1, quantTemas, "Tema escolhido invalido, digite novamente: ");
	...
	// Para tal, copiamos os valores da proxima linha na linha que sera sobrescrita.
	private static void deletarTema(int posicaoTema) {
		
		// Serao deslocadas as linhas a partir da linha que queremos sobrescrever.
		for (int i = posicaoTema; i < temasEPalavras.length; i++) {
			
			// Somente serao deslocadas as linhas que tiverem valores.
			if (temasEPalavras[i][0] != null) {
				for (int j = 0; j < temasEPalavras[i].length; j++) {
					
					// O uso do try and catch e para o caso do programa ter todos os temas possiveis, assim, para que o iltimo tema nao de erro,
					// fazemos isso, e colocamos null nessa ultima linha.
					try {
	...
	public static boolean temaSemPalavras(int posicaoTema) {
		...
		int primeiraPalavra = 1; // E o indice da primeira palavra do tema.
	...
	private static int posicaoTema(String tema) {
		...
		for (int i = 0; i < temasEPalavras.length; i++) {
			// Só faremos a repeticao enquanto houver valores nos temas.
			if (temasEPalavras[i][0] == null)
				break;
	...
	public static void gerenciarPalavras() {
		...
		do{
			...
			opcao = Utensilios.validarVariavel(1, 5, "Opção invalida, digite novamente: ");
			System.out.println();
	...
	private static void cadastrarPalavra() {
		...
		if (palavraRepetida(novaPalavra, palavrasDoTema))
			System.out.println("A palavra ja esta cadastrada para esse tema!");
		else {
	...
	private static boolean palavraRepetida (String novaPalavra, String[] listaPalavras) {
		...
		// Iniciamos a partir do 1, pois o primeiro valor da listaPalavras e o nome do tema.
	...
	private static void excluirPalavra() {
		...
		if (palavraEncontrada(palavra)) {
			String[] temasDaPalavra = temasPalavra(palavra); // Guarda o nome de todos os temas que contem a palavra.
	...
	private static void deletaPalavra (String palavra, int posicaoTema) {
		...
		for (int i = posicaoPalavra; i < listaPalavrasTema.length; i++) {
			// So percorre as colunas enquanto houver palavras com valores.
			if (listaPalavrasTema[i] == null)
				break;
			else {
				// // O uso do try and catch e para o caso do tema ter todas as palavras possiveis, assim, para que a ultimo palavra nao de erro,
				// fazemos isso, e colocamos null no ultimo item.
				try {
	...
	private static void excluirPalavraComVariosTemas(String palavra, String[] temasDaPalavra, int quantTemas) {
		System.out.println("Selecione em que tema a palavra será excluida:");
		System.out.println(" 0. Todos");
		imprimeTemasSelecionados(temasDaPalavra);
		
		int opcao = Utensilios.validarVariavel(0, quantTemas, "Opcao digitada invalida, tente novamente: ");
		switch(opcao) {
			case 0:
				...
				break;
			default:
				// Colocamos opcao - 1, pois o indice de temasDaPalavra inicia em 0, diferente do nosso menu que apresenta os temas da palavra. 
				int indexTema = opcao - 1;
				// Usamos posicaoTema() pois o índice do tema nao e necessariamente o mesmo da nossa array bidimensional.
	...
	private static boolean palavraEncontrada(String palavraBuscada) {
		...
		for (int i = 0; i < temasEPalavras.length; i++) {
			// Irá percorrer a array até encontrar um valor null, a partir desse momento nao haverá mais temas armazenados,
			// entao interrompe a repeticao.
			if (temasEPalavras[i][0] == null) {
				break;
			}else {
				// iniciamos a busca na coluna a partir do 1, pois o indice 0 é o nome do tema e percorremos somente os valores que nao
	...
	private static String[] temasPalavra (String palavraBuscada) {
		...
		for (int i = 0; i < temasEPalavras.length; i++) {
			// Irá percorrer a array até encontrar um valor null, a partir desse momento não havera mais temas armazenados,
			// então interrompe a repetição.
			if (temasEPalavras[i][0] == null) {
				break;
			}else {
				// iniciamos a busca na coluna a partir do 1, pois o indice 0 e o nome do tema e percorremos somente os valores que nao
				// forem null com o "arrayLengthNotNull".
				for (int j = 1; j < Utensilios.arrayLengthNotNull(temasEPalavras[i]); j++) {
					if (temasEPalavras[i][j].equals(palavraBuscada)) {
	...
	private static void listarPalavras() {
		...
		if (temaSemPalavras(posicaoTema))
			System.out.println("Esse tema nao tem palavras cadastradas!");
			...
			int quantPalavras = Utensilios.arrayLengthNotNull(listaPalavras); // Quantidade de palavras nao nulas na lista.
